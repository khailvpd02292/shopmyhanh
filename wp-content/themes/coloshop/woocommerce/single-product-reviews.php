		...
		<div id="comments">
			<h2 class="woocommerce-Reviews-title"><?php
				if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
					...
					printf( esc_html( _n( ' (%1$s) Review %2$s', '(%1$s) Reviews %2$s', $count, 'woocommerce' ) ), esc_html( $count ), '' );

					// printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'woocommerce' ) ), esc_html( $count ), '<span style="font-size:">' . get_the_title() . '</span>' );
				} else {
