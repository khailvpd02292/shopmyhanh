...
class installerWpf {
	...
	static private function _checkSendStat($statCode) {
		if(class_exists('frameWpf') 
			&& frameWpf::_()->getModule('supsystic_promo')
			&& frameWpf::_()->getModule('options')
		) {
			frameWpf::_()->getModule('supsystic_promo')->getModel()->saveUsageStat( $statCode );
			frameWpf::_()->getModule('supsystic_promo')->getModel()->checkAndSend( true );
		}
	}
