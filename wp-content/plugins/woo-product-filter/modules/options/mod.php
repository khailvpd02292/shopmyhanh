...
//		dispatcherWpf::addAction('afterModulesInit', array($this, 'initAllOptValues'));
	...
	public function getAdminPage() {
		if(installerWpf::isUsed()) {
			...
			frameWpf::_()->getModule('supsystic_promo')->getModel()->bigStatAdd('Welcome Show');
			frameWpf::_()->getModule('options')->getModel()->save('plug_welcome_show', time());	// Remember this

			return frameWpf::_()->getModule('supsystic_promo')->showWelcomePage();
		}
	}
...
//						'sendmail_path' => array('label' => __('Sendmail Path', WPF_LANG_CODE), 'desc' => __('You can check it on your server, or ask about it - in your hosting provider.', WPF_LANG_CODE), 'html' => 'text', 'connect' => 'mail_send_engine:sendmail', 'def' => $defSendmailPath),
			...
			$isPro = frameWpf::_()->getModule('supsystic_promo')->isPro();
			foreach($this->_options as $catKey => $cData) {
				foreach($cData['opts'] as $optKey => $opt) {
					...
					if(isset($opt['pro']) && !$isPro) {
						$this->_options[ $catKey ]['opts'][ $optKey ]['pro'] = frameWpf::_()->getModule('supsystic_promo')->generateMainLink('utm_source=plugin&utm_medium='. $optKey. '&utm_campaign=popup');
					}
				}
			}
